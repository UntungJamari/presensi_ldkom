...
if (isset($_GET['rpw']) && isset($_GET['no_anggota'])) {
    $no_anggota = $_GET['no_anggota'];
    $password = "12345678";

    $password = password_hash($password, PASSWORD_DEFAULT);

    $query = mysqli_query($koneksi, "update user set password = '$password' where no_anggota = '$no_anggota'");
    if ($query) {
        $berhasil = "Berhasil Mereset Password";
    } else {
        $gagal = "Gagal Mereset Password";
    }
}

if (isset($_GET['rf']) && isset($_GET['no_anggota'])) {
    $no_anggota = $_GET['no_anggota'];
    $foto = "default.jpg";

    $query = mysqli_query($koneksi, "update user set foto = '$foto' where no_anggota = '$no_anggota'");
    if ($query) {
        $berhasil = "Berhasil Mereset Foto";
    } else {
        $gagal = "Gagal Mereset Foto";
    }
}
...
if (isset($_POST['simpan'])) {
    if (empty($_POST["no_anggota"]) || empty($_POST["nama"])) {
        ...
        $query = mysqli_query($koneksi, "update user set nama = '$nama' where no_anggota = '$no_anggota'");
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <form action="edit_asisten.php" method="POST">
                                    <?php
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 pr-1">
                                            <div class="form-group">
                                                ...
                                                <a href="edit_asisten.php?rpw=yes&no_anggota=<?php echo $tampil['no_anggota']; ?>" onclick="return confirm('Apakah Anda yakin mereset password?')"><input type="button" class="form-control" value="Reset Password"></a>
                                            </div>
                                        </div>
                                        <div class="col-md-6 pl-1">
                                            <div class="form-group">
                                                <label>Foto (default : default.jpg)</label>
                                                <a href="edit_asisten.php?rf=yes&no_anggota=<?php echo $tampil['no_anggota']; ?>" onclick="return confirm('Apakah Anda yakin mereset foto?')"><input type="button" class="form-control" value="Reset Foto"></a>
                                            </div>
                                        </div>
                                    </div>
